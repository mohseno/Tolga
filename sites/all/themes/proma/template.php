...
function proma_pager($variables) {
  ...
  $li_first = theme('pager_first', array('text' => (isset($tags[0]) ? $tags[0] : t('« första')), 'element' => $element, 'parameters' => $parameters));
  $li_previous = theme('pager_previous', array('text' => (isset($tags[1]) ? $tags[1] : t('‹ föregående')), 'element' => $element, 'interval' => 1, 'parameters' => $parameters));
  $li_next = theme('pager_next', array('text' => (isset($tags[3]) ? $tags[3] : t('nästa ›')), 'element' => $element, 'interval' => 1, 'parameters' => $parameters));
  $li_last = theme('pager_last', array('text' => (isset($tags[4]) ? $tags[4] : t('sista »')), 'element' => $element, 'parameters' => $parameters));
